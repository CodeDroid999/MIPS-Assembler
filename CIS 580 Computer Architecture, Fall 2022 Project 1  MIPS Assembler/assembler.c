...
{
	/*char in[100] = "test1.asm";
	char out[100] = "test2.asm";
	argc = 3;
	argv[1] = in;
	argv[2] = out;*/
	currInstruction = 0;
	...
	preprocessed = fopen("program-name.out", "ab+");
	...
	FILE *preData = fopen("preDattest1.asm", "ab+");
...
// copy one register array to another array
void copyReg(int* source, int* dest){
	for(int i = 0; i < 5; i++){
	      dest[i] = source[i];
	}
